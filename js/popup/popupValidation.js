...
// document.addEventListener('DOMContentLoaded', function () {
//   // Инициализация каждой формы отдельно
//   document.querySelectorAll('.popup__form').forEach(form => {
...
//     // Валидация текстовых полей и textarea
//     form.querySelectorAll('input[type="text"], textarea').forEach(input => {
//       if (!input.value.trim()) {
//         showError(input, 'Это поле обязательно для заполнения');
//         isValid = false;
//       } else if (input === nameInput && !/^[-' a-zA-Zа-яА-ЯёЁ]+$/.test(input.value.trim())) {
//         showError(input, 'Имя содержит недопустимые символы');
//         isValid = false;
//       } else if (input === phoneInput) {
//         const phoneDigits = input.value.replace(/\D/g, '');
//         if (phoneDigits.length !== 11) {
//           showError(input, 'Номер телефона должен содержать 11 цифр');
//           isValid = false;
//         }
//       }
//     });
...
//     // Валидация чекбокса политики
//     if (policyCheckbox && !policyCheckbox.checked) {
//       showError(policyCheckbox, 'Необходимо согласие с политикой');
//       isValid = false;
//     }
...
//     const formData = new FormData(form);
//     formData.delete('policy'); // Удаляем policy из FormData, если он есть
